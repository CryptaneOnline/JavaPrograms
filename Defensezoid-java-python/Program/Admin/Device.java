...
class Device
{
    ...
    static Device getConnectedDevice()throws Exception
    {
        ...
        ec.waitFor();
        ec=Runtime.getRuntime().exec("adb shell \"service call iphonesubinfo 1 | cut -c 52-66 | tr -d \'.[:space:]\'\"");
        BufferedReader br=new BufferedReader(new InputStreamReader(ec.getInputStream()));        
        String imei=br.readLine();
        ec.waitFor();
        if(imei.contains("not found"))
        {
            ec=Runtime.getRuntime().exec("adb shell \"dumpsys iphonesubinfo | grep Device\"");
